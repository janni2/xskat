...
void mischen()
{
  ...
  if (wieder)
  {
    ...
    if (wieder == 1)
    {
      ...
      for (i = 0; i < 10; i++)
        swap(&cards[i], &cards[10 + i]);
      for (i = 0; i < 10; i++)
        swap(&cards[10 + i], &cards[20 + i]);
    }
    else if (wieder == 3)
    {
      if (vorhandwn)
      ...
      for (i = 0; i < 10; i++)
        swap(&cards[i], &cards[20 + i]);
      for (i = 0; i < 10; i++)
        swap(&cards[20 + i], &cards[10 + i]);
    }
    wieder = 0;
  }
  else if (!get_game())
  {
    do
    {
      for (i = 0; i < 32; i++)
        cards[i] = i;
      for (i = 0; i < 32; i++)
        swap(&cards[i], &cards[rndval(&seed[0], 31)]);
      for (i = 0; i < 10; i++)
        swap(&cards[geber * 10 + i], &cards[i]);
      for (i = 0; i < 10; i++)
...
void calc_rw(s) int s;
{
  ...
  if (!maxrw[s] &&
    ...
    maxrw[s] = 17;
  stg = strateg[numsp == 0 ? s : numsp == 1 ? s - 1
                                            : 0];
  if (stg < 0 && rnd(3) < -stg)
  {
    if (maxrw[s] > 17)
...
void truempfe()
{
  ...
  for (c = 0; c < 2; c++)
  {
    if ((cards[30 + c] & 7) == BUBE || cards[30 + c] >> 3 == trumpf)
    {
      for (i = 0; i < 10; i++)
      {
        if ((cards[10 * spieler + i] & 7) != BUBE && cards[10 * spieler + i] >> 3 != trumpf)
        {
...
int bb, b[4], vh;
{
  ...
  for (i = 0; i < 4; i++)
  {
    ...
    if ((ih > 4) ||
        (ih > 3 && (inhand[i][AS] || inhand[i][ZEHN])))
      g4 = 1;
...
int check_bockevents()
{
  ...
  {
    if (!spgew && stsum == 60 && trumpf >= 0 && trumpf <= 4)
      e++;
  }
  if (bockevents & BOCK_BEI_GRANDHAND)
  {
    if (spgew && trumpf == 4 && handsp)
      e++;
  }
  m = 0;
  if (bockevents & BOCK_BEI_KONTRA)
  {
    if (!spgew && kontrastufe)
      e++, m = 1;
  }
...
void set_prot()
{
  ...
  prot2.handsp = handsp;
  prot2.ehsso =
      revolang ? handsp ? 1 : 0 : ouveang ? trumpf == -1 && handsp ? 5 : 4
                              : schwang   ? 3
                              : schnang   ? 2
                              : handsp    ? 1
                                          : 0;
...
void modsum(sm, gv, p, as, ae, ar, ad) int sm[3][3], gv[3][2], p, *as, *ae, *ar, *ad;
{
  int s, e, r, d;
...
void schmieren()
{
  ...
  for (i = 1; i < possc; i++)
  {
    ...
    if (wj == 2)
      ...
      wj = aw[cj >> 3];
    if (wi > wj || (vmh == 2 && wi == wj && !wi && (ci & 7) > (cj & 7)))
      j = i;
  }
  playcd = j;
}
...
int einstechen()
{
  ...
  if (!cardw[stcd[0] & 7] || !uebernehmen(0, 0, 0))
    return 0;
    ...
    else
    {
      ...
      else
      {
        ...
        if (!vmh)
        {
          if (trumpf == 4)
          {
            if ((ci & 7) != BUBE && hatnfb[spieler][ci >> 3])
              wi -= 30;
            if ((cj & 7) != BUBE && hatnfb[spieler][cj >> 3])
              wj -= 30;
          }
          else
          {
...
int hatas()
{
  ...
  if (!as || (stcd[0] & 7) == BUBE || f == trumpf ||
      cardw[stcd[0] & 7] > 4 || hatnfb[spieler][f])
    return 0;
  playcd = as - 1;
  return 1;
}
...
int restbeimir()
{
  ...
  for (i = 0; i < 4; i++)
  {
    if (i == trumpf || (hatnfb[s[0]][i] && hatnfb[s[1]][i]))
      continue;
    h = SIEBEN + 1;
    for (j = 0; j < 2; j++)
    {
...
void m_bhsp()
{
  ...
  if (fabwerfen())
    return;
  if (!uebernehmen(!higher(stcd[0], stcd[1]), 1, 0))
    schenke();
}
...
void m_bhns()
{
  ...
  if (spitzefangen())
    return;
  if (rnd(1) && karobubespielen())
    return;
  if (rnd(3) && schnippeln(1))
    return;
  if (higher(stcd[0], stcd[1]) ^ (spieler != ausspl))
  {
    if (!genugdrin() || !uebernehmen(spieler != ausspl, 1, 0))
      abwerfen();
  }
  else
  {
