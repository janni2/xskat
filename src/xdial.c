...
void hndl_btevent(sn, bt) int sn, bt;
{
  ...
  if (ob[bt].spec & OB_EXIT) {
    ...
    if (ob == dihand) {
      ...
      ag = 0;
      if (dispiel[6].spec & OB_SELECTED || dispiel[13].spec & OB_SELECTED)
        trumpf = -1;
      else if (dispiel[2].spec & OB_SELECTED)
        trumpf = 0;
      else if (dispiel[3].spec & OB_SELECTED)
        trumpf = 1;
      else if (dispiel[4].spec & OB_SELECTED)
        trumpf = 2;
      else if (dispiel[5].spec & OB_SELECTED)
        trumpf = 3;
      else
        trumpf = 4;
      if (!handsp && trumpf != -1 &&
          (dispiel[8].spec & OB_SELECTED || dispiel[9].spec & OB_SELECTED ||
      ...
      if (!ag && trumpf == -1 &&
          reizw[reizp] >
              nullw[dispiel[13].spec & OB_SELECTED
                        ? 4
      ...
      if (!ag) {
        if (dispiel[13].spec & OB_SELECTED)
          revolang = ouveang = 1;
        else if (dispiel[10].spec & OB_SELECTED)
          ouveang = schwang = schnang = 1;
        else if (dispiel[9].spec & OB_SELECTED)
          schwang = schnang = 1;
        else if (dispiel[8].spec & OB_SELECTED)
          schnang = 1;
      ...
      remove_di(sn);
      if (didicht[3].spec & OB_SELECTED || didicht[5].spec & OB_SELECTED) {
        spielendscr();
      ...
      if (!(digrafik[sn][3 + blatt[sn]].spec & OB_SELECTED) ||
        ...
        blatt[sn] = (digrafik[sn][3].spec & OB_SELECTED   ? 0
                     ...
                     : digrafik[sn][5].spec & OB_SELECTED ? 2
                                                          : 3);
        for (i = 0; i < 33; i++) {
          create_card(sn, i - 1);
        }
      ...
      remove_di(sn);
      sav = (((((((playramsch) * 3 + playkontra) * 3 + playbock) * 3 +
                spitzezaehlt) *
                   2 +
               revolution) *
                  2 +
      ...
      if (sav != (((((((playramsch) * 3 + playkontra) * 3 + playbock) * 3 +
                     spitzezaehlt) *
                        2 +
                    revolution) *
                       2 +
      ...
      abkuerz[sn] = (digeschwindigkeit[sn][14].spec & OB_SELECTED   ? 0
                     : digeschwindigkeit[sn][15].spec & OB_SELECTED ? 1
                                                                    : 2);
      ...
      remove_di(sn);
      sav = (((mbutton[sn]) * 3 + keyboard[sn]) * 2 + briefmsg[sn]) * 2 +
            trickl2r[sn];
      for (i = 0; i <= 5; i++) {
      ...
      if (sav != (((mbutton[sn]) * 3 + keyboard[sn]) * 2 + briefmsg[sn]) * 2 +
                     trickl2r[sn] &&
          !irc_play)
        save_opt();
    }
...
OBJECT* ob;
{
  if (irc_play && ob != diende[sn] && ob != diterm[sn] && ob != diproto[sn] &&
      ...
      ob != dicopyr[sn] && ob != dicopyralt[sn] && ob != digrafik[sn] &&
      ob != distrateg[sn] && ob != divarianten[sn] && ob != diramschopts[sn] &&
      ob != dibockevents[sn] && ob != digeschwindigkeit[sn] && ob != digui[sn])
    irc_sendbtev(sn, bt);
  hndl_btevent(sn, bt);
}
...
void draw_selpos(sn, f) int sn, f;
{
  ...
  if (selpos[sn].num) {
    ...
    draw_wedge(win[sn], bp, sn, f, selpos[sn].p[p].x2, selpos[sn].p[p].y1,
               s + 1);
    draw_wedge(win[sn], bp, sn, f, selpos[sn].p[p].x1, selpos[sn].p[p].y2,
               s + 2);
    draw_wedge(win[sn], bp, sn, f, selpos[sn].p[p].x2, selpos[sn].p[p].y2,
               s + 3);
    draw_wedge(bck[sn], bp, sn, f, selpos[sn].p[p].x1, selpos[sn].p[p].y1, s);
    draw_wedge(bck[sn], bp, sn, f, selpos[sn].p[p].x2, selpos[sn].p[p].y1,
               s + 1);
    draw_wedge(bck[sn], bp, sn, f, selpos[sn].p[p].x1, selpos[sn].p[p].y2,
               s + 2);
    draw_wedge(bck[sn], bp, sn, f, selpos[sn].p[p].x2, selpos[sn].p[p].y2,
               s + 3);
  }
}
    ...
    if (!dir) {
      ...
      for (;;) {
        ...
        if (bt == ob[0].spec)
          ...
          bt = ob[0].spec - 1;
        if (ob[bt].spec & (OB_BUTTON | OB_EXIT) && !ob_disabled(ob, bt) &&
            !(ob[bt].spec & OB_HIDDEN) &&
            (!(ob[bt].spec & OB_SELECTED) || ob[bt].next == OB_NONE)) {
          actbtn[sn] = bt;
          break;
        }
      }
  ...
  if (phase == REIZEN) {
    ...
    if (sn == spieler) {
      ...
      for (i = 1; i < 11; i++) {
        ...
        selpos[sn].p[i].x2 =
            desk[sn].playx + i * desk[sn].cardx - 1 - desk[sn].large * 2;
        selpos[sn].p[i].y2 =
            desk[sn].playy + desk[sn].cardh + 1 - desk[sn].large;
        selpos[sn].p[i].f = 2 | 4;
      }
    }
      ...
      for (k = 0; k < 10; k++) {
        for (j = 0; j < possc; j++) {
          if (possi[j] % 10 == k) {
            ...
            selpos[sn].p[i].y1 = desk[sn].playy - 2;
            selpos[sn].p[i].x2 = desk[sn].playx + (k + 1) * desk[sn].cardx - 1 -
                                 desk[sn].large * 2;
            selpos[sn].p[i].y2 =
                desk[sn].playy + desk[sn].cardh + 1 - desk[sn].large;
            selpos[sn].p[i].f = 2;
  ...
  for (i = 0; i < 2; i++) {
    if ((s[i] = laninvite[i])) {
      ...
      if ((d = XOpenDisplay(dn[i]))) {
        if (!XGetDefault(d, prog_name, "ready"))
          ok = 0;
        else
          s[i] = 0;
        XCloseDisplay(d);
...
OBJECT* ob;
{
  ...
  rt        = XCreateSimpleWindow(dpy[sn], win[sn], x - 1, y - dy - 1,
                                  ob[0].w * charw[sn], dy + ob[0].h * charh[sn], 1,
                                  fgpix[sn], bgpix[sn]);
  ob[0].y   = dy;
  ob[0].win = rt;
...
void findlastex(sn) int sn;
{
  ...
  for (i = ob[0].spec - 1; i; i--) {
    if (ob[i].spec & OB_EXIT && !ob_disabled(ob, i)) {
      actbtn[sn] = i;
      return;
    }
  }
...
OBJECT* ob;
{
  ...
  if (!i) {
    ...
    if (gfx3d[sn]) {
      ...
      XFillRectangle(dpy[sn], ob[i].win, gcxor[sn], 0, 0, ob[i].w * charw[sn],
                     ob[i].h * charh[sn]);
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    if (resdial[sn]) {
      for (i = 0; i < resdial[sn][0].spec; i++) {
        if (resdial[sn][i].win) {
          while (XCheckTypedWindowEvent(dpy[sn], resdial[sn][i].win, Expose,
                                        &event)) {
            if (!event.xexpose.count) {
    ...
    if (actdial[sn]) {
      for (i = 0; i < actdial[sn][0].spec; i++) {
        if (actdial[sn][i].win) {
          while (XCheckTypedWindowEvent(dpy[sn], actdial[sn][i].win, Expose,
                                        &event)) {
            if (!event.xexpose.count) {
...
void di_info(sn, th) int sn, th;
{
  ...
  for (s = 0; s < 3; s++) {
    if (s != sn) {
      ...
      x = (s == left(sn) ? desk[sn].com1x : desk[sn].com2x) +
          30 * desk[sn].f / desk[sn].q;
      y = (s == left(sn) ? desk[sn].com1y : desk[sn].com2y) +
          130 * desk[sn].f / desk[sn].q;
      ...
      if (numsp == 1) {
        if (phase >= SPIELEN && phase <= NIMMSTICH && s == spieler &&
            trumpf != 5) {
          prspnam(txt, s, lang[sn]);
        } else {
          *txt = 0;
      ...
      if ((phase == SPIELEN || phase == SCHENKEN) &&
          (numsp != 1 || !briefmsg[sn]) &&
          (stich == 1 || (s != th && !briefmsg[sn])) && s == spieler &&
          trumpf != 5) {
        v_gtext(sn, x, y, 0, clr);
...
void di_ansage() {
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    for (ln = 0; ln < NUM_LANG; ln++) {
      tt[sn].t[ln] = txt[sn][ln];
      strcpy(txt[sn][ln],
             textarr[revolang ? TX_REVOLUTION : trumpf_idx(sn, trumpf)].t[ln]);
      if (trumpf == -1) {
        if (!revolang) {
          if (ouveang) {
            strcat(txt[sn][ln], " ");
...
void di_ktrnext(sn, f) int sn, f;
{
  if (kontrastufe == 1) {
    ...
    if (numsp == 1 && iscomp(spieler) && briefmsg[0] && playkontra == 2 &&
        !sagte18[0]) {
      create_di(0, diansage);
    } else
      do_angesagt();
...
void di_buben() {
  dibuben[2].str =
      &textarr[blatt[spieler] >= 2 ? TX_UNTER_NICHT : TX_BUBEN_NICHT];
...
void list_fun(sn) int sn;
{
  ...
  for (i = splfirst[sn], j = 1; j < 13 && i < splstp; i++, j++) {
    ...
    for (k = 0; k < 4; k++) {
      ...
      sprintf(txt[sn][j][k], "%d",
              k == 3 ? e > 0 && r && !d ? -e : e : curr[k][cp]);
    }
    if ((cp == 1 || (r && !d)) && e > 0) {
...
void di_liste(sn, ini) int sn, ini;
{
  ...
  diliste[sn][65].str =
      splfirst[sn] + 12 >= splstp ? &textarr[TX_LOESCHEN] : &tt3;
  diliste[sn][65].spec =
      splfirst[sn] + 12 < splstp ||
              ((splstp ||
...
int fil;
{
  ...
  while (*txt) {
    if (unformatted) {
      if (spec & OB_BOLD)
        fprintf(f, "%c", ger_toupper(*txt));
      else if (spec & OB_UNDERLINED && *txt == ' ')
        fputc('_', f);
      else
        fputc(*txt, f);
    } else {
      if (spec & OB_BOLD)
        fprintf(f, "%c\b%c", *txt, *txt);
      else if (spec & OB_UNDERLINED)
        fprintf(f, "_\b%c", *txt);
      else
        fputc(*txt, f);
    }
        ...
        if (prot1.stichgem || protsort[sn]) {
          ...
          strcat(txt[sn][i][s][ln],
                 textarr[prot1.schenken
                             ? prot1.spieler == s ? TX_AKZEPTIERT : TX_SCHENKEN
                             : TX_PASSE]
                     .t[ln]);
...
void di_proto(sn, ini, log) int sn, ini, log;
{
  ...
  for (p = 0; p < 2; p++) {
    for (s = 0; s < 3; s++) {
      diproto[sn][2 + p * 3 + s].spec =
          prot1.verdopp[s] == 2 ? OB_CENTERED | OB_UNDERLINED : OB_CENTERED;
    }
  }
  ...
  for (ln = 0; ln < NUM_LANG; ln++) {
    ...
    if (prot1.trumpf == 5) {
      ...
      strcat(txt[sn][ln],
             textarr[prot1.revolution ? TX_REVOLUTION
                                      : trumpf_idx(sn, prot1.trumpf)]
                 .t[ln]);
    }
  }
  ...
  if (log) {
    ...
    if (prot1.stichgem || prot1.schenken) {
      if (prot1.trumpf != 5) {
        fprintf(f, "%s %s\n", textarr[TX_IM_SKAT_WAR].t[lang[sn]],
                imskw[sn][lang[sn]]);
        if (prot1.gereizt) {
          fprintf(f, "%s %s\n", textarr[TX_GEREIZT_BIS].t[lang[sn]], bis[sn]);
        }
      ...
      fprintf(f, "%s\n", txt[sn][lang[sn]]);
      if (diproto[sn][40].str != &textarr[TX_IM_SKAT_WAR] &&
          prot1.trumpf != 5) {
    ...
    diproto[sn][47].str =
        &textarr[(!sn || irc_play) && protsort[sn] ? TX_SPEICHERN
                                                   : TX_SPIELLISTE];
...
void di_resultdi(sn) int sn;
{
  ...
  if (spgew && (alist[sn] == 1 || (trumpf == 5 && stsum != 120))) {
...
void di_result(be) int be;
{
  ...
  diresult[2].str  = (trumpf == 5 ? (mes1   ? &textarr[TX_JUNGFRAU]
  ...
  for (ln = 0; ln < NUM_LANG; ln++) {
    ...
    if (be > 1)
      sprintf(txt[ln], textarr[TX_N_BOCK_EREIGNISSE].t[ln], be);
    else
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    if (numsp > 1) {
      ...
      if (actdial[sn]) remove_di(sn);
      create_dial(sn, (desk[sn].w - resdial[sn][0].w * charw[sn]) / 2, 5, 3,
                  resdial[sn]);
      ...
      if (sx && sy) {
        if ((trumpf == 5 && sn == ausspl) || handsp) {
          if (sx > desk[sn].w / 2) {
            sx -= (strlen(textarr[TX_IM_SKAT_IST].t[lang[sn]]) + 0) * charw[sn];
          }
          ...
          v_gtextnc(sn, 0, 0, sx, sy, 0,
                    textarr[TX_URSPRUENG_SKAT].t[lang[sn]]);
        }
      }
...
void di_options(sn) int sn;
{
  ...
  dioptions[sn][14].spec = dioptions[sn][15].spec = dioptions[sn][16].spec =
      (dioptions[sn][17].spec == OB_EXIT || dioptions[sn][18].spec == OB_EXIT)
          ? OB_HIDDEN
          : OB_NONE;
...
void di_grafik(sn) int sn;
{
  ...
  if (irc_play || numsp > 1) {
    digrafik[sn][10].spec = digrafik[sn][11].spec = digrafik[sn][12].spec =
        OB_HIDDEN;
  }
...
void di_geschwindigkeit(sn) int sn;
{
  ...
  for (i = 0; i < 3; i++) {
    ...
    digeschwindigkeit[sn][14 + i].spec =
        irc_play ? OB_DISABLED | OB_BUTTON : OB_BUTTON;
  }
