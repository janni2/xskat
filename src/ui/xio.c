...
XColor color[3][256] = {
    {
     ...
     {0, 0x0000, 0xb400, 0x0000}
        // Other elements of color[0] and all of color[1], color[2] will be
        // zero-initialized.
    }
};
...
int bigs[]       = {33, 60,  15, 6,  51, 6,   33, 24,  15, 43,  51, 43,  15, 77,
                    51, 77,  33, 93, 15, 111, 51, 111, 7,  9,   60, 107, 7,  9,
                    60, 107, 7,  9,  60, 107, 15, 6,   51, 6,   15, 43,  51, 43,
                    33, 60,  15, 77, 51, 77,  15, 111, 51, 111, 15, 6,   51, 6,
                    33, 33,  15, 60, 51, 60,  33, 87,  15, 111, 51, 111, 15, 6,
                    51, 6,   33, 33, 15, 60,  51, 60,  15, 111, 51, 111};
...
/* Definitions for info_reiz, trumpf_idx, info_spiel, info_stich, clear_info,
   set_names are intentionally removed from xio.c as they are (or should be) in
   xdial.c Their declarations should be in xdial.h or text.h, which are
   included.
*/
...
void drop_card(i, s) int i, s;
{
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    x2[sn] = desk[sn].stichx + (l2r[sn]         ? vmh
                                : s == left(sn) ? 0
                                : s == sn       ? 1
                                                : 2) *
                                   desk[sn].cardw;
...
XColor* xcol;
{
  ...
  for (sp = 0; sp < (1 << pl) / 32 + 1; sp++) {
    ...
    for (i = j + 1; i < 1 << pl; i++) {
      if (abs(xcm[sn][i].red - r) + abs(xcm[sn][i].green - g) +
              abs(xcm[sn][i].blue - b) <
          abs(xcm[sn][j].red - r) + abs(xcm[sn][j].green - g) +
              abs(xcm[sn][j].blue - b)) {
...
void calc_desk(sn) int sn;
{
  ...
  desk[sn].playx =
      desk[sn].x + (desk[sn].w - desk[sn].cardw - 9 * desk[sn].cardx) / 2;
...
void drawimg(sn, c, f, w, x, y) int sn, c, f, w, x, y;
{
  ...
  decompgif(f < 0            ? backsd_gif[0]
            : blatt[sn] >= 2 ? de_gif[f][w]
                             : fr_gif[f][w],
    ...
    for (i = 0; i <= 0xffff; i += 0x1111) {
      for (j = 0; j <= 0xffff; j += 0x1111) {
        for (k = 0; k <= 0xffff; k += 0x1111) {
          l  = 0;
          ld = abs(xcp[l].red - i) + abs(xcp[l].green - j) +
               abs(xcp[l].blue - k);
          for (p = 1; p < desk[sn].col; p++) {
            pd = abs(xcp[p].red - i) + abs(xcp[p].green - j) +
                 abs(xcp[p].blue - k);
            if (pd < ld) {
              l  = p;
              ld = pd;
  ...
  if (tc) {
    for (i = 0; i < cmapsize; i++) {
      ...
      xc.green = (*themap)[i][1];
      xc.blue  = (*themap)[i][2];
      if (blatt[sn] == 3 && f == 3 && xc.red > 220 && xc.green < 100 &&
          xc.blue < 100) {
  ...
  for (i = (desk[sn].large ? 87 : 86 + (f < 0)); i >= 0; i--) {
    ...
    for (j = (gr ? 137 : 68); j >= 0; j--) {
      ...
      if (xn[p] == 32) {
        ...
        XDrawPoints(dpy[sn], cardpx[sn][c + 1], gc[sn], xp[p], 32,
                    CoordModeOrigin);
        xn[p] = 0;
      }
    }
  ...
  for (p = 0; p < 256; p++) {
    if (xn[p]) {
      change_gc(sn, tc ? pm[p] : xcp[p].pixel, gc);
      XDrawPoints(dpy[sn], cardpx[sn][c + 1], gc[sn], xp[p], xn[p],
                  CoordModeOrigin);
    }
  }
}
...
void create_card(sn, c) int sn, c;
{
  ...
  if (((KOENIG <= w && w <= BUBE) || c < 0 || blatt[sn] >= 2)) {
    ...
    XFillRectangle(dpy[sn], cardpx[sn][c + 1], gc[sn], x + 1, y + 1,
                   60 * desk[sn].f / desk[sn].q - 2,
                   92 * desk[sn].f / desk[sn].q);
  }
  ...
  for (i = 0; i < (desk[sn].large ? 9 : 5); i++) {
    ...
    XDrawPoint(dpy[sn], cardpx[sn][c + 1], gc[sn],
               x + ww - frm[desk[sn].large][i][0],
               y + hh - frm[desk[sn].large][i][1]);
    XDrawPoint(dpy[sn], cardpx[sn][c + 1], gc[sn],
               x + frm[desk[sn].large][i][0],
               y + hh - frm[desk[sn].large][i][1]);
    XDrawPoint(dpy[sn], cardpx[sn][c + 1], gc[sn],
               x + ww - frm[desk[sn].large][i][0],
               y + frm[desk[sn].large][i][1]);
  }
  ...
  if (c >= 0) {
    if (blatt[sn] < 2) {
      change_gcxor(
          sn, (upf ? color[sn][pf].pixel : bpix[sn]) ^ wpix[sn] ^ bgpix[sn]);
      p = cnts[w];
      do {
        ...
        if (!desk[sn].large) {
          x3 = x3 * 2 / 3 +
               (KOENIG <= w && w <= BUBE && p - 2 != cnts[w] ? 1 : 0);
          y3 = y3 * 2 / 3 +
               (KOENIG <= w && w <= BUBE && p - 2 != cnts[w] ? 1 : 0);
        }
        if (upf) {
          XCopyPlane(dpy[sn], symbs[sn], cardpx[sn][c + 1], gcxor[sn],
                     x1 + 2 * desk[sn].cardw, y1, x2 - x1, y2 - y1, x + x3,
                     y + y3, 1);
        } else {
          XCopyPlane(dpy[sn], symbs[sn], cardpx[sn][c + 1], gcxor[sn], x1, y1,
                     x2 - x1, y2 - y1, x + x3, y + y3, 1);
        }
      } while (p != cnts[w + 1]);
    } else {
    ...
    for (p = 0; p < 8; p += 2) {
      ...
      if (blatt[sn] < 2) {
        x1 = 256 + (w == AS     ? lang[sn] * 20
                    : w == ZEHN ? p > 3 ? 23 : 0
                    : w <= BUBE ? (w - KOENIG + 1) * 5 + lang[sn] * 20
                                : (p > 3 ? 31 : 8) + (w - NEUN) * 5);
      } else {
        x1 = 256 + (w == ZEHN   ? p > 3 ? 23 : 0
                    : w <= BUBE ? (w - KOENIG + 1) * 5 + 35
                                : (p > 3 ? 31 : 8) + (w - NEUN) * 5);
      }
...
char** argv;
{
  ...
  win[sn] = XCreateSimpleWindow(dpy[sn], DefaultRootWindow(dpy[sn]), desk[sn].x,
                                desk[sn].y, desk[sn].w, desk[sn].h, 0,
                                fgpix[sn], bgpix[sn]);
  ...
  if (desk[sn].col > 3) {
    icon =
        XCreatePixmap(dpy[sn], win[sn], icon_width, icon_height, desk[sn].plan);
    ...
    XCopyPlane(dpy[sn], symbs[sn], iconmask, igc[sn], 128 + 16, 0, 16, 16, 16,
               16, 1);
    XCopyPlane(dpy[sn], symbs[sn], iconmask, igc[sn], 128 + 32, 0, 16, 16, 16,
               1, 1);
    XCopyPlane(dpy[sn], symbs[sn], iconmask, igc[sn], 128 + 48, 0, 16, 16, 1,
               16, 1);
    ...
    XCopyPlane(dpy[sn], symbs[sn], icon, gcxor[sn], 128 + 16, 0, 16, 16, 16, 16,
               1);
    change_gcxor(sn, color[sn][2].pixel ^ wpix[sn] ^ bgpix[sn]);
    XCopyPlane(dpy[sn], symbs[sn], icon, gcxor[sn], 128 + 32, 0, 16, 16, 16, 1,
               1);
    change_gcxor(sn, color[sn][3].pixel ^ wpix[sn] ^ bgpix[sn]);
    XCopyPlane(dpy[sn], symbs[sn], icon, gcxor[sn], 128 + 48, 0, 16, 16, 1, 16,
               1);
    change_gcxor(sn, fgpix[sn]);
    change_gc(sn, fgpix[sn], gc);
  } else {
  ...
  XSetStandardProperties(dpy[sn], win[sn], title[sn], title[sn], icon, argv,
                         argc, &szhints[sn]);
#ifdef PWinGravity
  szhints[sn].win_gravity =
      (geom_f[sn] & XNegative
           ? (geom_f[sn] & YNegative ? SouthEastGravity : NorthEastGravity)
           : (geom_f[sn] & YNegative ? SouthWestGravity : NorthWestGravity));
  ...
  bck[sn] =
      XCreatePixmap(dpy[sn], win[sn], desk[sn].w, desk[sn].h, desk[sn].plan);
  if (bck[sn] == None) nomem();
  for (i = 0; i < 33; i++) {
    cardpx[sn][i] = XCreatePixmap(dpy[sn], win[sn], desk[sn].cardw,
                                  desk[sn].cardh, desk[sn].plan);
...
void card_colors(sn) int sn;
{
  ...
  for (; c < 4; c++) {
    ...
    for (i = 0; i < ncol; i++) {
      ...
      if (j == desk[sn].col) {
        for (j = i + 1; j < ncol && color[sn][desk[sn].col + j].pixel != p;
             j++);
...
void xinitres(sn) int sn;
{
  ...
  calc_desk(sn);
  if (!title[sn] && !(title[sn] = XGetDefault(dpy[sn], prog_name, "title")))
    title[sn] = prog_name;
  if (!geom_f[sn] && (res = XGetDefault(dpy[sn], prog_name, "geometry"))) {
  ...
  if (!keyboardset[sn] && (res = XGetDefault(dpy[sn], prog_name, "keyboard"))) {
    keyboard[sn] = atoi(res);
  }
  if (keyboard[sn] < 0 || keyboard[sn] > 2) keyboard[sn] = 1;
  if (!abkuerzset[sn] && (res = XGetDefault(dpy[sn], prog_name, "shortcut"))) {
    abkuerz[sn] = atoi(res);
    if (abkuerz[sn] < 0 || abkuerz[sn] > 2) abkuerz[sn] = 1;
  }
  if (!tdelayset[sn] && (res = XGetDefault(dpy[sn], prog_name, "tdelay"))) {
    nimmstich[sn][0] = (int)(atof(res) * 10 + .5);
  }
  if (nimmstich[sn][0] < 0)
    nimmstich[sn][0] = 7;
  else if (nimmstich[sn][0] > 101)
    nimmstich[sn][0] = 101;
  if (!downupset[sn]) {
  ...
  if (!sn) {
    ...
    if (!rskatloserset &&
        (res = XGetDefault(dpy[sn], prog_name, "skattoloser"))) {
    ...
    if (!spitzezaehltset && (res = XGetDefault(dpy[sn], prog_name, "spitze"))) {
      spitzezaehlt = atoi(res);
      if (spitzezaehlt < 0)
        spitzezaehlt = 0;
      else if (spitzezaehlt > 2)
        spitzezaehlt = 2;
    }
    if (!revolutionset &&
    ...
    if (!klopfenset && (res = XGetDefault(dpy[sn], prog_name, "klopfen"))) {
      klopfen = istrue(res);
    }
    if (!schenkenset && (res = XGetDefault(dpy[sn], prog_name, "schenken"))) {
      schenken = istrue(res);
    }
    if (!oldrulesset && (res = XGetDefault(dpy[sn], prog_name, "oldrules"))) {
      oldrules = istrue(res);
    }
    if (irc_play < 0) {
    ...
    if (irc_play) {
      game_file = 0;
      if (irc_pos < 0 && (res = XGetDefault(dpy[sn], prog_name, "ircpos"))) {
        irc_pos = atoi(res);
      }
      irc_pos--;
      ...
      if (!irc_nick &&
          ...
          !(irc_nick = getenv("IRCNICK")) && !(irc_nick = getenv("IRCUSER")) &&
  ...
  if ((geom_f[sn] & (XValue | YValue)) == (XValue | YValue)) {
    szhints[sn].x = geom_f[sn] & XNegative
                        ? XDisplayWidth(dpy[sn], scr) + geom_x[sn] - desk[sn].w
                        : geom_x[sn];
    szhints[sn].y = geom_f[sn] & YNegative
                        ? XDisplayHeight(dpy[sn], scr) + geom_y[sn] - desk[sn].h
                        : geom_y[sn];
  ...
  for (ln = 0; ln < NUM_LANG; ln++) {
    for (i = 0; i < TX_NUM_TX; i++) {
      if ((len = strlen(textarr[i].t[ln])) > 3 &&
          (nw = (XTextWidth(dfont[sn], textarr[i].t[ln], len) + len - 1) /
                len) > cw) {
        cw = nw;
      }
    }
...
void read_cards() {
  ...
  for (cd = 0; cd < sizeof(map_gif) / sizeof(map_gif[0]); cd++) {
    ...
    if (bufidx >= buflen || feof(f) || strcmp(buf + cdst, "GIF87aX") ||
...
void drawcard(sn, c, x, y) int sn, c, x, y;
{
  XCopyArea(dpy[sn], cardpx[sn][c + 1], win[sn], gc[sn], 0, 0, desk[sn].cardw,
            desk[sn].cardh, x, y);
  XCopyArea(dpy[sn], cardpx[sn][c + 1], bck[sn], gc[sn], 0, 0, desk[sn].cardw,
            desk[sn].cardh, x, y);
}
...
GC* gcp;
{
  ...
  xyarr[1] =
      (c < 30 ? desk[sn].playy : desk[sn].skaty) - 5 * desk[sn].f / desk[sn].q;
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    if (s < 0) {
      ...
      if (!n)
        ...
        x1[sn] = desk[sn].playx + 3 * desk[sn].cardx;
      else
        x1[sn] = desk[sn].playx + 7 * desk[sn].cardx;
      y1[sn] = desk[sn].playy;
    }
    x2[sn] = desk[sn].x + desk[sn].w;
    if (geber != sn) {
...
void spielendscr() {
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    if (sn != spieler || abkuerz[sn] == 2) {
      ...
      v_gtext(
          sn, x, y1, 0,
          textarr[trumpf == -1 ? TX_NULL_DICHT : TX_REST_BEI_MIR].t[lang[sn]]);
    }
  }
  phase = WEITER;
  di_weiter(1);
...
void revolutionscr() {
  ...
  for (sn = 0; sn < numsp; sn++) {
    ...
    v_gtextnc(sn, 0, 0, x, desk[sn].com1y + desk[sn].cardh + 1, 0,
              textarr[spieler == ausspl         ? TX_VORHAND
                      : spieler == left(ausspl) ? TX_MITTELHAND
                                                : TX_HINTERHAND]
                  .t[lang[sn]]);
    v_gtextnc(sn, 0, 0, x, desk[sn].skaty - charh[sn] - 1, 0,
              textarr[mi == ausspl         ? TX_VORHAND
...
void draw_box(sn, x, y, w) int sn, x, y, w;
{
  ...
  xy[1] = y - 1;
  xy[2] = x + w - 3;
  xy[3] = y + charh[sn] - gfx3d[sn] + 1;
  if (gfx3d[sn]) {
    draw_3d(win[sn], bck[sn], sn, xy[0], xy[1], xy[2], xy[3], 0);
  } else {
    XDrawRectangle(dpy[sn], win[sn], gc[sn], xy[0], xy[1], xy[2] - xy[0],
                   xy[3] - xy[1]);
    XDrawRectangle(dpy[sn], bck[sn], gc[sn], xy[0], xy[1], xy[2] - xy[0],
                   xy[3] - xy[1]);
  }
  xy[0]++;
  xy[1]++;
  ...
  xy[1]++;
  change_gc(sn, btpix[sn], gc);
  XFillRectangle(dpy[sn], win[sn], gc[sn], xy[0], xy[1], xy[2] - xy[0],
                 xy[3] - xy[1]);
  XFillRectangle(dpy[sn], bck[sn], gc[sn], xy[0], xy[1], xy[2] - xy[0],
                 xy[3] - xy[1]);
  change_gc(sn, fgpix[sn], gc);
}
...
void put_box(s) int s;
{
  ...
  for (sn = 0; sn < numsp; sn++) {
    if (s != sn) {
      if (s == left(sn))
        draw_box(sn, desk[sn].cbox1x, desk[sn].cboxy, desk[sn].cardw);
      else
        draw_box(sn, desk[sn].cbox2x, desk[sn].cboxy, desk[sn].cardw);
    } else {
      draw_box(sn, desk[sn].pboxx, desk[sn].pboxy, desk[sn].cardw);
      draw_box(sn, desk[sn].pboxx + desk[sn].cardw, desk[sn].pboxy,
               desk[sn].cardw);
    }
  }
}
...
void rem_box(s) int s;
{
  ...
  for (sn = 0; sn < numsp; sn++) {
    if (s != sn) {
      if (s == left(sn))
        backgr(sn, desk[sn].cbox1x, desk[sn].cboxy - 5,
               66 * desk[sn].f / desk[sn].q, 28 * desk[sn].f / desk[sn].q);
      else
        backgr(sn, desk[sn].cbox2x, desk[sn].cboxy - 5, desk[sn].cardw,
               28 * desk[sn].f / desk[sn].q);
    } else {
      backgr(sn, desk[sn].pboxx, desk[sn].pboxy - 5, desk[sn].cardw,
             28 * desk[sn].f / desk[sn].q);
...
void inv_box(s, c, rev) int s, c, rev;
{
  ...
  for (sn = 0; sn < numsp; sn++) {
    x = (s != sn ? s == left(sn) ? desk[sn].cbox1x : desk[sn].cbox2x
                 : desk[sn].pboxx + (c ? desk[sn].cardw : 0)) +
        4;
...
int sn, x, y;
{
  ...
  if (c) {
    ...
    } else if (hintcard[0] == drkcd + 30)
      hintcard[0] = 10 * sn + c;
    if (hintcard[1] == 10 * sn + c) {
      hintcard[1] = drkcd + 30;
      if (hints[sn]) show_hint(sn, 1, 0);
    } else if (hintcard[1] == drkcd + 30)
      hintcard[1] = 10 * sn + c;
    sna[0] = sn;
