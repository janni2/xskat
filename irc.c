...
void irc_pr_alist(val) int val;
{
  irc_pr_ss(textarr[TX_SPIELLISTE].t[lang[0]],
            textarr[val == 2 ? TX_TURNIER : val ? TX_ALTERNATIV
                                                : TX_NORMAL]
                .t[lang[0]]);
}
...
void irc_showrules(aplayramsch, aplaysramsch, aplaykontra, aplaybock,
                   ...
                   aoldrules, arskatloser) int aplayramsch,
...
char *s;
{
  ...
  {
  case 0:
    return !strcmp(irc_conf[1].nick, s) ? 2 : !irc_2player && !strcmp(irc_conf[2].nick, s) ? 3
                                                                                           : 0;
  case 1:
    return !strcmp(irc_conf[0].nick, s) ? 1 : !irc_2player && !strcmp(irc_conf[2].nick, s) ? 3
                                                                                           : 0;
  default:
    return !strcmp(irc_conf[0].nick, s) ? 1 : !strcmp(irc_conf[1].nick, s) ? 2
                                                                           : 0;
  }
}
...
char *p;
{
  if (irc_history[sn][idx])
    ...
    irc_histcnt[sn]++;
  if (!(irc_history[sn][idx] = (char *)malloc(strlen(p) + 1)))
    nomem();
