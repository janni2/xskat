...
XColor *xcol;
{
  ...
  if (XAllocColor(dpy[sn], cmap[sn], xcol))
    return 1;
  pl = desk[sn].plan;
  if (pl < 2 || pl > 8 || wpix[sn] >= 1 << pl || bpix[sn] >= 1 << pl)
    return 0;
  if (!f[sn])
  {
    for (i = 0; i < 256; i++)
...
unsigned char *thegif;
{
  ...
  if (csiz >= 8)
    c += (long)thegif[pos + 2] << 16;
  c >>= *bpos & 7;
  *bpos += csiz;
  return c & msk;
}

void decompgif(thedata, thepic, themap, cmapsize) unsigned char *thedata, *thepic, **themap;
int *cmapsize;
{
  unsigned char b, *p;
...
void drawimg(sn, c, f, w, x, y) int sn, c, f, w, x, y;
{
  ...
  decompgif(f < 0 ? backsd_gif[0] : blatt[sn] >= 2 ? de_gif[f][w]
                                                   : fr_gif[f][w],
...
void create_card(sn, c) int sn, c;
{
  ...
  if (c >= 0)
  {
    ...
    for (p = 0; p < 8; p += 2)
    {
      ...
      if (blatt[sn] < 2)
      {
        x1 = 256 + (w == AS ? lang[sn] * 20 : w == ZEHN ? p > 3 ? 23 : 0
...
char **argv;
{
  ...
  szhints[sn].win_gravity =
      (geom_f[sn] & XNegative ? (geom_f[sn] & YNegative ? SouthEastGravity : NorthEastGravity) : (geom_f[sn] & YNegative ? SouthWestGravity : NorthWestGravity));
  ...
  {
    XSelectInput(dpy[sn], win[sn], ButtonPressMask | ExposureMask | KeyPressMask | KeyReleaseMask);
    ...
    for (i = 0; i < ncol; i++)
    {
      ...
      if (j == desk[sn].col)
      {
        for (j = i + 1; j < ncol && color[sn][desk[sn].col + j].pixel != p; j++)
          ;
        if (j == ncol)
          XFreeColors(dpy[sn], cmap[sn], &p, 1, 0);
      }
    }
  }
}
...
void xinitres(sn) int sn;
{
  ...
  if (gfx3d[sn])
  {
    bgpix[sn] = get_col(sn, bg_col, prog_name, "3dbackground",
                        desk[sn].plan > 1 ? "#cccccccccccc" : borw == wpix[sn] ? "white"
                                                                               : "black",
                        borw, &nocol);
    btpix[sn] = get_col(sn, bt_col, prog_name, "3dbutton",
                        desk[sn].plan > 1 ? "#dddddddddddd" : borw == wpix[sn] ? "white"
                                                                               : "black",
                        borw, &nocol);
  ...
  calc_desk(sn);
  if (!title[sn] &&
      !(title[sn] = XGetDefault(dpy[sn], prog_name, "title")))
    title[sn] = prog_name;
  if (!geom_f[sn] &&
  ...
  if (keyboard[sn] < 0 || keyboard[sn] > 2)
    keyboard[sn] = 1;
  if (!abkuerzset[sn] &&
      (res = XGetDefault(dpy[sn], prog_name, "shortcut")))
  {
    abkuerz[sn] = atoi(res);
    if (abkuerz[sn] < 0 || abkuerz[sn] > 2)
      abkuerz[sn] = 1;
  }
  if (!tdelayset[sn] &&
  ...
  if (!sn)
  {
    ...
    if (prot_file && !*prot_file)
      prot_file = 0;
    if (opt_file && !*opt_file)
      opt_file = 0;
    if (game_file && !*game_file)
      game_file = 0;
    if (list_file && !*list_file)
      list_file = 0;
    if (irc_logfile && !*irc_logfile)
      irc_logfile = 0;
    if (cards_file && !*cards_file)
      cards_file = 0;
  }
  if ((geom_f[sn] & (XValue | YValue)) == (XValue | YValue))
  {
    szhints[sn].x = geom_f[sn] & XNegative ? XDisplayWidth(dpy[sn], scr) + geom_x[sn] - desk[sn].w : geom_x[sn];
    szhints[sn].y = geom_f[sn] & YNegative ? XDisplayHeight(dpy[sn], scr) + geom_y[sn] - desk[sn].h : geom_y[sn];
...
GC *gcp;
{
  ...
  xyarr[1] = (c < 30 ? desk[sn].playy : desk[sn].skaty) - 5 * desk[sn].f / desk[sn].q;
...
void putamark(sn, s) int sn, s;
{
  ...
  if (sn == s || ouveang || (spitzeang && !sptzmrk))
    return;
...
void givecard(s, n) int s, n;
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    if (s == sn)
    {
      ...
      if (n == 1)
        putback(sn, x1[sn] + 3 * desk[sn].cardx, y1[sn]);
    }
    else if (s < 0)
    {
...
void initscr(sn, sor) int sn, sor;
{
  ...
  if (sor)
  {
    ...
    for (i = 0; i < 10; i++)
    {
      if (c0 >= 0 && c0 == cards[sn * 10 + i])
        hintcard[0] = sn * 10 + i;
      if (c1 >= 0 && c1 == cards[sn * 10 + i])
        hintcard[1] = sn * 10 + i;
...
void spielendscr()
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    if (sn != spieler || abkuerz[sn] == 2)
    {
      ...
      v_gtext(sn, x, y1, 0, textarr[trumpf == -1 ? TX_NULL_DICHT : TX_REST_BEI_MIR].t[lang[sn]]);
    }
  }
  phase = WEITER;
  di_weiter(1);
...
void revolutionsort(sp) int sp;
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    for (i = 0; i < 10; i++)
    {
      ...
      s = left(sn);
      if (sp || s != spieler)
        putcard(sn, cards[s * 10 + i], x, y1);
      s = left(s);
      if (sp || s != spieler)
        putcard(sn, cards[s * 10 + i], x, y2);
    }
  }
}
...
void revolutionscr()
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    v_gtextnc(sn, 0, 0, x, desk[sn].com1y + desk[sn].cardh + 1,
              0, textarr[spieler == ausspl ? TX_VORHAND : spieler == left(ausspl) ? TX_MITTELHAND
                                                                                  : TX_HINTERHAND]
                     .t[lang[sn]]);
    v_gtextnc(sn, 0, 0, x, desk[sn].skaty - charh[sn] - 1,
              0, textarr[mi == ausspl ? TX_VORHAND : mi == left(ausspl) ? TX_MITTELHAND
                                                                        : TX_HINTERHAND]
                     .t[lang[sn]]);
    if (f)
    {
...
void put_box(s) int s;
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    {
      if (s == left(sn))
        draw_box(sn, desk[sn].cbox1x, desk[sn].cboxy,
                 desk[sn].cardw);
      else
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    if (s != sn)
    {
      if (s == left(sn))
        ...
        backgr(sn, desk[sn].cbox2x, desk[sn].cboxy - 5,
...
int sn, x, y, opt, send;
{
  ...
  {
    if (sn == spieler)
      ok = hndl_druecken(sn, x, y);
  }
  else if (phase == SPIELEN)
  {
    if (sn == (ausspl + vmh) % 3)
      ok = hndl_spielen(sn, x, y);
  }
  else if (phase == NIMMSTICH)
  {
    if (nimmstich[sn][1])
      ok = hndl_nimmstich(sn);
  }
  else if (phase == REVOLUTION)
  {
    if (sn == tauschply)
      ok = hndl_tauschen(sn, x, y);
  }
  if (!ok)
  {
    if (irc_play && sn != irc_pos)
      return ok;
    if (opt == 1)
    {
      di_options(sn);
...
void setcurs(f) int f;
{
  ...
  if (newsn != actsn)
  {
    wtime = 0;
    if (actsn >= 0)
      XDefineCursor(dpy[actsn], win[actsn], cursor[actsn][1]);
    actsn = newsn;
    if (actsn >= 0)
    {
      for (sn = 0; sn < numsp; sn++)
  ...
  if (actsn == -1)
    wtime = 0;
  if (phase == REIZEN && bockspiele && !ramschspiele)
    wtime = 15 * 1000;
  if (!wtime || wtime >= 15 * 1000)
  {
                      ...
                      (phase != REIZEN && phase != SPIELEN && phase != NIMMSTICH)))
    {
      ...
      if (wtime == 16 * 1000)
        wtime -= 1500;
      if (phase != REIZEN && phase != SPIELEN && phase != NIMMSTICH)
        wtime = 0;
    }
    if (wtime == 15 * 1000 &&
        (phase == REIZEN || phase == SPIELEN || phase == NIMMSTICH))
