...
int sn, tr;
{
  return blatt[sn] >= 2 && tr >= 0 && tr <= 3 ? TX_SCHELLEN + tr : TX_NULL + tr + 1;
}
    ...
    for (bl = 0; bl < 2; bl++)
    {
      for (ln = 0; ln < NUM_LANG; ln++)
      {
        ...
        sprintf(txt[bl][sn][ln], "%s %s", textarr[TX_GESPIELT_WIRD].t[ln],
                textarr[revolang ? TX_REVOLUTION : bl && trumpf >= 0 && trumpf <= 3 ? TX_SCHELLEN + trumpf
                                                                                    : TX_NULL + trumpf + 1]
                    .t[ln]);
      }
    }
  ...
  if (ob[bt].spec & OB_EXIT)
  {
    ...
    else if (ob == dispiel)
    {
      ...
      if (dispiel[6].spec & OB_SELECTED ||
        ...
        trumpf = 4;
      if (!handsp && trumpf != -1 &&
          (dispiel[8].spec & OB_SELECTED || dispiel[9].spec & OB_SELECTED || dispiel[10].spec & OB_SELECTED))
      {
        create_di(sn, difehler);
        ag = 1;
      }
      if (!ag && trumpf == -1 &&
          reizw[reizp] > nullw[dispiel[13].spec & OB_SELECTED ? 4 : (dispiel[10].spec & OB_SELECTED ? 2 : 0) + handsp])
      {
    ...
    else if (ob == dioptions[sn])
    {
      ...
      if (s1 != sort1[sn] || s2 != sort2[sn] || al != alternate[sn])
      {
        ...
        if (irc_play)
          irc_sendsort(sn);
        initscr(sn, 1);
        if (sav != sort1[sn] + 2 * alternate[sn] && !irc_play)
          save_opt();
      }
      if (bt == 21)
        di_ende(sn);
    ...
    else if (ob == digrafik[sn])
    {
          ...
          (sav / 4 != lang[sn] && blatt[sn] < 2))
      {
        if (!ag)
          remove_di(sn);
        blatt[sn] = (digrafik[sn][3].spec & OB_SELECTED ? 0 : digrafik[sn][4].spec & OB_SELECTED ? 1
      ...
      if (ag)
      {
        ...
        set_conames();
        if (sav != blatt[sn] + 4 * lang[sn] && !irc_play)
          save_opt();
        di_grafik(sn);
      }
      if (bt == 11)
    ...
    {
      remove_di(sn);
      sav = (((((((playramsch) * 3 + playkontra) * 3 + playbock) * 3 + spitzezaehlt) * 2 + revolution) * 2 + klopfen) * 2 + schenken) * 2 + oldrules;
      for (i = 0; i < 3; i++)
      {
      ...
      if (sav != (((((((playramsch) * 3 + playkontra) * 3 + playbock) * 3 + spitzezaehlt) * 2 + revolution) * 2 + klopfen) * 2 + schenken) * 2 + oldrules)
      {
        save_opt();
      }
      if (playramsch)
    ...
    else if (ob == diramschopts[sn])
    {
      ...
      if (sav != playsramsch * 2 + rskatloser)
        save_opt();
      if (playbock)
        di_bockevents(sn);
      else
    ...
    else if (ob == dibockevents[sn])
    {
      ...
      if (sav != bockevents * 2 + resumebock)
        save_opt();
      di_geschwindigkeit(sn);
    }
    else if (ob == digeschwindigkeit[sn])
    {
      ...
      case 7:
        nimmstich[sn][0]++;
        if (irc_play && nimmstich[sn][0] >= 101)
          nimmstich[sn][0] = 100;
        break;
      case 8:
        nimmstich[sn][0] += 10;
      ...
      abkuerz[sn] = (digeschwindigkeit[sn][14].spec & OB_SELECTED ? 0 : digeschwindigkeit[sn][15].spec & OB_SELECTED ? 1
                                                                                                                     : 2);
    ...
    else if (ob == dieigenertisch)
    {
      ...
      if (sav != laninvite[0] * 2 + laninvite[1])
        save_opt();
      if (bt == 13)
        di_lanspiel(sn);
    ...
    {
      remove_di(sn);
      sav = (((mbutton[sn]) * 3 + keyboard[sn]) * 2 + briefmsg[sn]) * 2 + trickl2r[sn];
      for (i = 0; i <= 5; i++)
      {
      ...
      if (sav != (((mbutton[sn]) * 3 + keyboard[sn]) * 2 + briefmsg[sn]) * 2 + trickl2r[sn] && !irc_play)
        save_opt();
    }
  }
}
...
void set_selpos(sn) int sn;
{
  ...
  if (selpos[sn].num || actdial[sn] || !keyboard[sn])
    return;
  ...
  else if (phase == DRUECKEN)
  {
    if (sn == spieler)
    {
      ...
      if (hints[sn] && hintcard[0] != -1)
      {
        selpos[sn].act = 0;
        if (hintcard[0] < 30)
          selpos[sn].act = hintcard[0] - 10 * sn + 1;
        if (hintcard[1] < 30)
          selpos[sn].act = hintcard[1] - 10 * sn + 1;
      }
      selpos[sn].num = 11;
      i = 0;
...
int i, *x, *y, *w, *h, *bp;
{
  ...
  bd = ob[i].spec & OB_BUTTON ? 1 : ob[i].spec & OB_EXIT ? 2
                                                         : 0;
  ba = gfx3d[sn] ? 2 * bd : 0;
  d3d = bd && gfx3d[sn] ? 1 : 0;
...
OBJECT *ob;
{
  ...
  for (i = 1; i < ob[0].spec; i++)
  {
    ...
    ob[i].win = wi;
    XSelectInput(dpy[sn], wi, ExposureMask | (ob[i].spec & (OB_BUTTON | OB_EXIT) ? ButtonPressMask : 0));
...
int sn, x1, y1, x2, y2, rev;
{
  ...
  if (b != None)
    XDrawLines(dpy[sn], b, gc[sn], pts, 3, CoordModeOrigin);
  pts[0].x++;
  pts[1].x = (short)x2;
  pts[1].y = (short)y2;
  pts[2].y++;
    ...
    if (gfx3d[sn])
    {
      ...
      if (sel || !ob_disabled(ob, i))
      {
        if (bd)
        {
          ...
          if (bd == 2)
            draw_3d(ob[i].win, None, sn, 1, 1, w - 2, h - 2, sel);
        }
      }
    }
    else if (sel)
  ...
  for (s = 0; s < 3; s++)
  {
    if (s != sn)
    {
          ...
          (stich == 1 || (s != th && !briefmsg[sn])) && s == spieler && trumpf != 5)
      {
        ...
        strcat(txt, textarr[revolang ? TX_REVOLUTION : trumpf_idx(sn, trumpf)].t[lang[sn]]);
        v_gtext(sn, x, y, 0, txt);
        y += charh[sn];
        v_gtext(sn, x, y, 0, clr);
        if (ouveang && !revolang)
      ...
      else if (th >= 0)
      {
        ...
        {
          v_gtext(sn, x, y, 0, textarr[ausspl == s ? TX_VORHAND : (ausspl + 1) % 3 == s ? TX_MITTELHAND
                                                                                        : TX_HINTERHAND]
                                   .t[lang[sn]]);
        }
        v_gtext(sn, x, y + charh[sn], 0, clr);
...
void di_ansage()
{
  ...
  for (sn = 0; sn < numsp; sn++)
  {
    ...
    {
      tt[sn].t[ln] = txt[sn][ln];
      strcpy(txt[sn][ln], textarr[revolang ? TX_REVOLUTION : trumpf_idx(sn, trumpf)].t[ln]);
      if (trumpf == -1)
      {
        if (!revolang)
        {
...
void di_liste(sn, ini) int sn, ini;
{
  ...
  if (ini)
    splfirst[sn] = ((splstp > 0 ? splstp - 1 : 0) / 12) * 12;
      ...
      for (ln = 0; ln < NUM_LANG; ln++)
      {
        ...
        else
        {
          if (e == OB_UNDERLINED)
            ...
            txt[sn][i][s][ln][0] = 0;
          strcat(txt[sn][i][s][ln],
                 textarr[prot1.schenken ? prot1.spieler == s ? TX_AKZEPTIERT : TX_SCHENKEN : TX_PASSE].t[ln]);
          if (e == OB_UNDERLINED)
            strcat(txt[sn][i][s][ln], " ");
        }
      }
...
int i;
{
  strcpy(s, textarr[(blatt[sn] >= 2 ? TX_SCHELLEN : TX_KARO) + (prot1.skat[i][0] >> 3)].t[ln]);
  strcat(s, textarr[(blatt[sn] >= 2 ? TX_AD : TX_A) + (prot1.skat[i][0] & 7)].t[ln]);
  strcat(s, ",");
  strcat(s, textarr[(blatt[sn] >= 2 ? TX_SCHELLEN : TX_KARO) + (prot1.skat[i][1] >> 3)].t[ln]);
...
void di_proto(sn, ini, log) int sn, ini, log;
{
  ...
  for (p = 0; p < 2; p++)
  {
    for (s = 0; s < 3; s++)
    {
      diproto[sn][2 + p * 3 + s].spec = prot1.verdopp[s] == 2 ? OB_CENTERED | OB_UNDERLINED : OB_CENTERED;
    }
  }
  ...
  for (ln = 0; ln < NUM_LANG; ln++)
  {
    ...
    else
    {
      ...
      strcat(txt[sn][ln], textarr[prot1.revolution ? TX_REVOLUTION : trumpf_idx(sn, prot1.trumpf)].t[ln]);
    }
  }
  ...
  {
    p = protsort[sn];
    if (prot1.verdopp[prot1.anspiel[0]] == 1)
      u1 = OB_UNDERLINED;
    if (prot1.verdopp[left(prot1.anspiel[0])] == 1)
      u2 = OB_UNDERLINED;
    if (prot1.verdopp[right(prot1.anspiel[0])] == 1)
      u3 = OB_UNDERLINED;
  }
  else
  {
...
void di_result(be) int be;
{
  ...
  diresult[2].str =
      (trumpf == 5 ? (mes1 ? &textarr[TX_JUNGFRAU] : mes2 ? &textarr[TX_DURCHMARSCH]
                                                          : OB_NONE)
